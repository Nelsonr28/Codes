...
/*void ListaDoble::InsertForma_B (string &nombre, int &poder, int &edad, int &num_po, string &descripcion)
{
  ...
  if (nombre < pri -> nombre)                           //Si no esta vacia entonces...
    {
      nuevo -> pro = pri;
...
void ListaDoble::InsertForma_B (string &nombre, int &poder, int &edad, int &num_po, string &descripcion){
      ...
      if (EsVacia()){
           return ;
            ...
            cout << "nombre." << endl;
              ...
              cin >> act->edad;
                ...
                getline (cin, act->descripcion);
                q=pri;

               while(q!=NULL)
               {
                      if ((q->nombre.compare(act->nombre)>0) && q->ant==NULL)  // q= aux; act= aux1                    //Si no esta vacia entonces...
                  {
                      q->pro= nuevo;
                      nuevo->ant= q;
                      ult= nuevo;
       ...
       if ((q->nombre.compare(act->nombre))<0 && q->pro==NULL)// INSERTA EN LOS MEDIOS.
                       {
                          q->pro= nuevo;
                          nuevo->ant=act;
                          nuevo->pro= ult;
                      ...
                      if (q->nombre.compare(act->nombre)>0) // lo hicimos en la biblioteca.
                      {
                        q->pro=nuevo;
                        nuevo->ant=q;
                        q=nuevo;
